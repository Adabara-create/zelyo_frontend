...
class _WithdrawScreenState extends ConsumerState<WithdrawScreen> {
  ...
  // The wallet is now controlled by accountsProvider, which is the single
  // source of truth for balances across Home, Transfer, Deposit, Withdraw,
  // Convert, and the other wallet screens.
  List<TransferCurrency> get _currencies =>
      ref.read(accountsProvider);

  // Store only the selected currency code so we can always resolve the
  // latest version of that account from Riverpod.
  late String _selectedCurrencyCode;

  /// Gets the latest selected currency from Riverpod.
  TransferCurrency get _selectedCurrency {
    return ref.read(
      accountByCodeProvider(_selectedCurrencyCode),
    );
  }
  ...
  void initState() {
    ...
    // Initialize the selected currency from the shared provider.
    _selectedCurrencyCode = _currencies.first.code;
  ...
  double get _fee {
    ...
    return _selectedCurrency.code == 'NGN'
        ? 50
        : 2.0;
  }

  double get _netAmount =>
      (_amount - _fee).clamp(0, double.infinity);

  bool get _exceedsBalance =>
      _amount > _selectedCurrency.balance;

  bool get _canWithdraw =>
      _amount > 0 && !_exceedsBalance;
  ...
  Future<void> _pickCurrency() async {
    ...
    if (picked != null) {
      setState(() {
        _selectedCurrencyCode = picked.code;
      });
    }
  }
  ...
  Future<void> _pickBankAccount() async {
    final picked =
        await showModalBottomSheet<_BankAccount>(
      context: context,
      backgroundColor: Colors.transparent,
      isScrollControlled: true,
      builder: (context) => _BankAccountPickerSheet(
  ...
  Future<void> _onWithdrawTap() async {
    ...
    // -------------------------------------------------------------------------
    //
    // The PIN has successfully been entered.
    //
    // This is a normal user-event callback, so changing the provider here is
    // safe. We deduct the requested withdrawal amount from the selected
    // currency wallet.
  ...
  Widget build(BuildContext context) {
    // Watch the shared wallet state so this screen rebuilds when another
    // screen changes a balance.
        ...
        children: [
          _SummaryRow(
            label:
                'Withdrawal amount',
            value:
                '${_selectedCurrency.symbol}${formatAmount(_amount)}',
          ),
          const SizedBox(height: 12),
          _SummaryRow(
            label: 'Fee',
            value:
                '${_selectedCurrency.symbol}${formatAmount(_fee)}',
          ),
          const Padding(
            padding:
          ...
          _SummaryRow(
            ...
            value:
                '${_selectedCurrency.symbol}${formatAmount(_netAmount)}',
            isTotal: true,
          ),
        ],
      ),
...
class _AuthorizeWithdrawalSheet
  ...
  @override
  State<_AuthorizeWithdrawalSheet>
      createState() =>
          _AuthorizeWithdrawalSheetState();
}

class _AuthorizeWithdrawalSheetState
    extends State<_AuthorizeWithdrawalSheet>
  ...
  Future<void> _verifyPin() async {
    ...
    if (_pin == _correctPin) {
      // Only the parent screen changes the Riverpod state.
      // This sheet simply reports successful authorization.
