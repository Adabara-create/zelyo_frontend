...
class _DepositScreenState extends ConsumerState<DepositScreen> {
  ...
  // ---------------------------------------------------------------------------
  //
  // The original screen had a hardcoded currency list here.
  // This getter keeps the existing `_currencies` name throughout the screen
  // while making accountsProvider the single source of truth.
  List<TransferCurrency> get _currencies =>
      ref.read(accountsProvider);

  // Store the selected currency by code so the selected account always comes
  // back from Riverpod with its latest balance.
  late String _selectedCurrencyCode;

  /// Gets the currently selected currency from the shared Riverpod state.
    ...
    return (
      bankName:
          code == 'NGN'
              ...
              : 'Zelyo Partner Bank',
      accountNumber:
          '90${code.codeUnits.fold<int>(0, (a, b) => a + b) % 90000000 + 1000000}',
  ...
  void _onAddMoneyTap() {
    ...
    // -------------------------------------------------------------------------
    //
    // This is a user action, so changing the provider here is safe.
    //
    // The amount is added to the selected wallet and every screen watching
    // accountsProvider will automatically receive the updated balance.
  ...
  Widget build(BuildContext context) {
    // Watch the shared wallet state so the Deposit screen rebuilds whenever
    // another screen changes a currency balance.
  ...
  Widget _buildHeader() {
    return Padding(
      padding:
          const EdgeInsets.fromLTRB(12, 8, 20, 4),
      child: Row(
        children: [
          IconButton(
            onPressed: () =>
...
class _ChangeCardSheet extends StatefulWidget {
  ...
  @override
  State<_ChangeCardSheet> createState() =>
      _ChangeCardSheetState();
}

class _ChangeCardSheetState
    extends State<_ChangeCardSheet> {
