...
/// Riverpod state manager for notifications.
///
/// This becomes the single source of truth for notification state.
/// Later the [build] method can be replaced with a backend / push
/// notification fetch without changing [NotificationsScreen].
class NotificationsNotifier extends Notifier<List<_NotificationItem>> {
  @override
  List<_NotificationItem> build() {
    return _buildSampleNotifications();
  }
  ...
  List<_NotificationItem> _buildSampleNotifications() {
    ...
    return [
      ...
      _NotificationItem(
        ...
        message:
            'Your account was accessed from a new iPhone in Lagos.',
        timestamp: now.subtract(
      ...
      _NotificationItem(
        ...
        message:
            'Transfers may be briefly delayed tonight between 1–2 AM.',
        timestamp: now.subtract(
      ...
      _NotificationItem(
        ...
        message:
            '₦20,000 was withdrawn at a GTBank ATM.',
        timestamp: now.subtract(
      ...
      _NotificationItem(
        ...
        message:
            'You earned ₦850 cashback on your Netflix payment.',
        timestamp: now.subtract(
      ...
      _NotificationItem(
        ...
        message:
            'Send money to any Zelyo user for free, this weekend only.',
        timestamp: now.subtract(
  ...
  /// Whether at least one notification is unread.
  bool get hasUnread =>
      state.any((item) => !item.isRead);

  /// Number of unread notifications.
  int get unreadCount =>
      state.where((item) => !item.isRead).length;
  ...
  /// Useful later when notifications come from your backend.
...
/// Global Riverpod notification provider.
///
/// HomeScreen, NotificationsScreen, notification badges, and any future
/// notification widgets can all watch this same provider.
final notificationsProvider =
    NotifierProvider<
      NotificationsNotifier,
      List<_NotificationItem>
    >(
...
class _NotificationsScreenState
  ...
  Widget build(BuildContext context) {
    // Watch the notification provider so this screen rebuilds whenever:
    // - a notification is marked read
    // - all notifications are marked read
    // - a notification is dismissed
    // - a new notification is added
    final notifications =
